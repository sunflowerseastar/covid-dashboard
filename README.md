...
* [`src/clj/covid_dashboard/`](src/clj/covid_dashboard/): CSS compilation source files (Clojure,
[Garden](https://github.com/noprompt/garden))
* [`src/cljs/covid_dashboard/`](src/cljs/covid_dashboard/): SPA source files (ClojureScript,
[re-frame](https://github.com/Day8/re-frame))
  - [`core.cljs`](src/cljs/covid_dashboard/core.cljs): contains the SPA entry point, `init`
* [`test/cljs/covid_dashboard/`](test/cljs/covid_dashboard/): test files (ClojureScript,
...
(headless test environment)
    * For Chromium, set the `CHROME_BIN` environment variable in your shell to the command that
    launches Chromium. For example, in Ubuntu, add the following line to your `.bashrc`:
       ...
       ```
7. Clone this repo and open a terminal in the `covid-dashboard` project root directory
...
in the [`src/clj/covid_dashboard/`](src/clj/covid_dashboard/) directory. CSS files are compiled
...
The `debug?` variable in [`config.cljs`](src/cljs/covid_dashboard/config.cljs) defaults to `true` in
[`dev`](#running-the-app) builds, and `false` in [`prod`](#production) builds.

Use `debug?` for logging or other tasks that should run only on `dev` builds:

```clj
(ns covid-dashboard.example
  (:require [covid-dashboard.config :as config])
