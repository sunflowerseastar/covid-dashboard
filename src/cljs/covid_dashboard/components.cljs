...
(defn detail
  ...
  (let [county (re-frame/subscribe [:detail-county])
    ...
    [box :class (str "u-position-relative z-index-1 opacity-0-fade-duration-2 " (if (or @county @country) "opacity-1" "pointer-events-none"))
     :child (if (or @county @country)
              [:div.panel.detail.z-index-1.padding-2.opacity-0-fade-duration-2 {:class (if is-switching "pointer-events-none" "opacity-1")}
               [:table.detail-table
    ...
    [:div.u-width-100.u-height-100
     [:div.u-position-relative.u-width-100.u-height-100
      ...
      [v-box :size "1" :class "u-height-100" :children
       [(if @mobile?
          ;; display (mobile)
          [box :size "1" :class (str "u-width-100 u-height-100 justify-content-center opacity-0-fade-duration-2 " (if @is-switching "pointer-events-none" "opacity-1"))
      ...
      [v-box :size "1" :class "u-absolute-all pointer-events-none" :children
       [[box :size "1" :child ""]
        [box :size "0 1 auto" :child
         [:div.menu-container.pointer-events-auto.opacity-0-fade-duration-2.scroll-y-auto {:class (if @is-menu-active "opacity-1" "pointer-events-none")}
     ...
     ;; display (desktop)
     (when (not @mobile?) [box :size "1" :class (str "u-width-100 u-height-100 justify-content-center opacity-0-fade-duration-2 " (if @is-switching "pointer-events-none" "opacity-1"))
                           :attr {:on-click #(reset! is-menu-active false)} :child [(-> (get sub-panels (mod @curr sub-panel-count)) second)]])]))
    ...
    [:div.u-position-relative.u-width-100.u-height-100
     ...
     [v-box :size "1" :class "u-absolute-all pointer-events-none" :children
      [[box :size "1" :class "blur" :child ""]
       [box :child [:div.menu-container.pointer-events-auto.opacity-0-fade-duration-2 {:class (if @is-menu-active "opacity-1" "pointer-events-none")}
