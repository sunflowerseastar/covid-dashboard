...
(defn bubble-map-population [starting-width]
  (-> (.all js/Promise (map #(.json js/d3 %) files-population))
      (.then
       (fn [[population-data counties-albers-10m-data]]
         (let [myGeoPath (geoPath)
               ...
               svg (.. js/d3 (select "#bubble-map-population-d3-svg-root") (attr "width" starting-width) (attr "height" (* starting-width 0.6)))
...
(defn bubble-map-population-d3 []
  (r/create-class
   {:display-name "bubble-map-population-d3"
